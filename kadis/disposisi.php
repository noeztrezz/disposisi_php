            ...
            <div class="box-body">
              <table class="table table-striped tabl-bordered nowrap" style="width:100%" id="myTable">
                            ...
                            <tbody>
                                <?php
                                include "../koneksi.php";
                                $sql = "SELECT * FROM dis_surat INNER JOIN surat where idpenerima=".$_SESSION['idpenerima']." AND id_proses=".$_SESSION['idpengirim']." OR idpengirim=".$_SESSION['idpenerima']." AND id_proses=".$_SESSION['idpengirim'];
                                $hasil = $con->query($sql);
                                ...
                                if ($hasil == FALSE){
                                    ...
                                    while ($h = $hasil->fetch_array()){
                                        ...
                                        <tr>
                                         ...
                                         <td><?php if (is_null($h['idpengirim'])) {
                                           # code...
                                         }else{
                                          $sqlx = $con->query("SELECT * FROM sub_bidang WHERE id_sub=".$h['idpengirim']);
                                           $sin = mysqli_fetch_array($sqlx);
                                           echo $sin['nama_sub'];
                                         } ?></td>
                                         <td><?php if (is_null($h['idpenerima'])) {
                                           # code...
                                         }else{
                                          $sqlx = $con->query("SELECT * FROM sub_bidang WHERE id_sub=".$h['idpenerima']);
                                           $sin = mysqli_fetch_array($sqlx);
                                           echo $sin['nama_sub'];
                                         } ?></td>
                                         <td><p><?php echo $h['tanggal'] ?></p></td>
                                         <td><?php echo $h['isi_disposisi'] ?></td>
