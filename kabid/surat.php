                                ...
                                if ($hasil == FALSE){
                                    ...
                                    while ($h = $hasil->fetch_array()){
                                        ...
                                        <tr>
                                         ...
                                         <td><?php echo $h['isi_singkat'] ?></td>
                                         <td><?php echo $h['tanggal_surat'] ?></td>
                                         <td><a href="<?php echo $h['lampiran'] ?>">Klik Disini</a></td>
                                         <td><?php
                                         if ($h['status']=="Belum Diproses") {
                                           echo "<button type='button' class='btn btn-danger' data-toggle='modal' data-target='#exampleModal' data-whatever=".$h['id_surat']."><i>Disposisi</i></button> </button> <button type='button' class='btn btn-warning' data-toggle='modal' data-target='#tesModal' data-kepo=".$h['id_surat']."><i>Kembalikan</i></button>";
                                         }elseif ($h['status']=="Validasi Kabid") {
                                           echo "<button type='button' class='btn btn-danger' data-toggle='modal' data-target='#kirimModal' data-kirim=".$h['id_surat']."><i>Disposisi</i></button> </button> <button type='button' class='btn btn-warning' data-toggle='modal' data-target='#tesModal' data-kepo=".$h['id_surat']."><i>Kembalikan</i></button>";
                                         }elseif ($h['status']=="Dikembalikan") {
                                           echo "<button type='button' class='btn btn-danger' data-toggle='modal' data-target='#exampleModal' data-whatever=".$h['id_surat']."><i>Kirim Ulang</i></button>";
                                         }elseif ($h['status']=="Dikembalikan Atasan") {
                                           echo "<button type='button' class='btn btn-danger' data-toggle='modal' data-target='#kirimModal' data-kirim=".$h['id_surat']."><i>Kirim Ulang</i></button>";
                                         } else {
                                           echo "<a href='#' class='btn btn-success btn-sm'><i class='fas fa-pencil-alt'>Sudah Disposisi</i></a>";
                                         }
...
<div class="modal fade" id="kirimModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="prosesdisposisi2.php" method="post">
          <div class="form-group">
            <input type="hidden" name="id_surat" id="recipient-name">
            <label for="recipient-name" class="col-form-label">Tujuan Disposisi</label>
            <select class="form-control" name="id_penerima">
              ...
              <?php
                include '../koneksi.php';
                $query = "select * from sub_bidang where id_bidang=2";
                $hasil = mysqli_query($con,$query);
                while($data=mysqli_fetch_array($hasil)){
                  echo "<option value=$data[id_sub]>$data[nama_sub]</option>";
                }
...
<script>
  ...
  var button = $(event.relatedTarget) // Button that triggered the modal
  var proses = button.data('kirim') // Extract info from data-* attributes
  // If necessary, you could initiate an AJAX request here (and then do the updating in a callback).
  // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
  var modal = $(this)
  modal.find('.modal-body input').val(proses)
