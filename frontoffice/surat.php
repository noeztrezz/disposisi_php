  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><i class="fa fa-envelope"> Data Surat</i></li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <center><h3 class="box-title"><a href="tambahsurat.php" class="btn btn-primary btn-sm"><i class="fas fa-pencil-alt">Tambah Surat</i></a> <a href="pdf.php" class="btn btn-primary btn-sm"><i class="fas fa-pencil-alt">Cetak Laporan</i></a></h3></center>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table class="table table-striped tabl-bordered nowrap" style="width:100%" id="myTable">
                            ...
                            <tbody>
                                <?php
                                include "../koneksi.php";
                                $sql = "SELECT * FROM surat where id_input=".$_SESSION['idpenerima'];
                                $hasil = $con->query($sql);
                                ...
                                if ($hasil == FALSE){
                                    ...
                                    while ($h = $hasil->fetch_array()){
                                        ...
                                        <tr>
                                         ...
                                         <td><?php 
                                         if (is_null($h['id_penerima'])) {
                                            echo $h['id_penerima'];
                                          }else{
                                            $sqlx = $con->query("SELECT * FROM sub_bidang WHERE id_sub=".$h['id_penerima']);
                                           $sin = mysqli_fetch_array($sqlx);
                                           echo $sin['nama_sub'];
                                          } ?></td>
                                         <td><p><?php echo $h['judul'] ?></p></td>
                                         <td><p><?php echo $h['status']." - ".$h['catatan'] ?></p></td>
                                         <td><p><a class="btn btn-danger btn-sm" href="<?php echo $h['lampiran'] ?>">Klik Disini</a></p></td>
                                         <td><p><?php
                                         if ($h['status']=="Selesai") {
                                           echo "<a href='#' class='btn btn-success btn-sm'><i class='fas fa-pencil-alt'>Selesai</i></a>";
                                          }  else if ($h['status']=="Ditolak") {
                                           echo "<a href='ubahsurat.php?id=$h[id_surat]' class='btn btn-danger btn-sm'><i class='fas fa-pencil-alt'>Perbaiki Surat</i></a>";
                                          }  else if ($h['status']=="Belum Didata") {
                                           echo "<a href='#' class='btn btn-warning btn-sm'><i class='fas fa-pencil-alt'>Belum Didata</i></a>";
                                          } else if($h['status']=="Belum Dikirim"){
                                            ...
                                            echo "<a href='#' class='btn btn-danger btn-sm'><i class='fas fa-pencil-alt'>Sedang Diproses</i></a>";
                                          }
                                         
                                         ?></p></td>
                                         <td><p><?php echo $h['tanggal_input'] ?></p></td>
